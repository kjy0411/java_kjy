...
*/
/*
회원 : 아이디(PK), 비번, 전화번호, 권한(USER, ADMIN), 대출권수(최대 3권), 정지기간
도서 : 도서번호(PK), 도서명, 저자, 분류, 출판사, 상태(대출 가능, 대출 중, 예약 중)
예약 : 예약번호(PK), 예약 신청일, 예약 마감일(예약일+3), 예약상태(대출 완료 ,취소됨), 아이디(FK), 도서번호(FK)
대출 : 대출번호(PK), 대출일, 반납 예정일(대출일+14), 예약번호(FK)
반납 : 반납번호(PK), 반납일, 연체기간, 아이디(FK), 대출번호(FK)
*/
