...
BEGIN
    ...
    -- => A영화 예매율 : A영화 예매 좌석수 / 예매중인 전체 예매 좌석수 * 100
    -- 전체 예매 좌석수
    SET _TOTAL_SEAT = (
		SELECT 
...
CREATE PROCEDURE RESERVATION_MOVIE(
    ...
    IN _SEAT_LIST TEXT		-- 좌석번호를 JSON 형태의 문자열로 받아옴
)
BEGIN
    ...
    */
    ...
    SET _RV_NUM = CONCAT(
        ...
        -- DATE_FORMAT(YYYY-MM-DD HH:MM:SS, '%Y%m%d%H%i') : YYYY-MM-DD HH:MM:SS => YYYYMMDDHHMM
	...
	-- 전체 요금을 계산 :
    -- 성인수, 청소년수, 조조할인 여부 알면 => 청소년 총 요금, 성인 총 요금을 꼐산
	SELECT _RV_NUM;
	-- 예약 테이블에 정보를 추가
    ...
    READ_LOOP : LOOP
        ...
        SET _SE_NUM = (
			SELECT SE_NUM FROM SEAT
				...
				JOIN MOVIE_SCHEDULE ON SC_NUM = MS_SC_NUM
				WHERE MS_NUM = _MS_NUM AND SE_NAME = SEAT_NAME
		);
        -- 예약좌석에 가격 번호를 연결하는데 있어서, 청소년 먼저하고 성인을 하는 것과
        -- 성인 먼저하고 청소년을 먼저하는 것은 중요하지 않음
