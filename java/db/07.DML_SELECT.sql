...
as : 속성 또는 테이블명을 일시적으로 변경해서 사용할 대 쓰는 키워드
속성명1 as 속성명2 => 속성명1을 일시적으로 속성명 2로 사용
*/
use course;
...
like: 와일드카드와 함께 사용되며, 값이 주어진 와일드가 포함된 문자열과 같은지를 비교할 때 사용
_ : 한글자를 의미하는 와일드 카드
% : 0글자이상을 의미하는 와일드 카드
속성명 between A and B : 속성의 값이 A이상 B이하인 조건을 만족할 때 사용
=> 속성명 >= A and 속성명 <= B
속성명 in(값1, 값2, ..., 값n) : 속성의 값이 값1이거나 값2거나 ... 값n인 조건을 만족할 때 사용
...
select * from student where name like '유__'; -- 성이 유씨이고 이름이 3글자인 학생의 정보 조회
select * from student where name like '유%';	-- 성이 유씨인 학새의 정보 조회
...
-- 학점이 2학점 또는 3학점 이상인 과목의 수강정보를 조회
SELECT 
    *
from course where subject_code = any (select code from subject where point in(2 , 3));
-- 글쓰기와 영어가 아닌 모든 과복의 수강 정보를 조회
...
-- 학생정보를 학번순으로 오름차순 정렬 후 위에서 2명의 학생 정보를 조회하는 쿼리
select * from student order by num asc limit 2;
-- 학생들 정보를 학번순으로 오름차순 정렬 후 3번째 4번째 학생 정보를 조회하는 쿼리
select * from student limit 2, 2;
-- 학생들 정보를 학번순으로 정렬 후, 2명씩 보여줄 때 3번째 페이지에 있는 학생 정보를 조회하는 쿼리
select * from student limit 4/*(3-1)*2인 값*/, 2;
