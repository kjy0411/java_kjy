...
요구사항
- 회원은 회원가입을 통해 프로그램을 이용한다.
- 회원 : 회원번호, 이름, 전화번호를 입력한다.
- 제품은 관리자가 등록한다.
- 제품은 제품번호, 가격, 재고, 제품분류를 입력한다.  
- 구매내역은 날짜, 회원번호, 제품번호, 개수, 비용으로 나타낸다.

요구사항
- 판매자는 1명으로 고정. 판매자는 관리자이다.
...
- 동일 제품(옵션까지 동일)을 장바구니에 담는 경우 최근에 담은 수량으로 변경된다.
...
- 제품 구매가 완료되면 주문번호, 구매한 제품 목록(제품명, 제품코드, 옵션, 수령,), 배송지를 보여준다.
...
- 회원은 반품/ 환불을 위해 하자가 있는 제품의 이미지를 관리자에 등록해서 승인을 받아야 환불이 진행된다.
- 회원은 제품을 받고 제품에 문제가 없으면 구매 확정을 통해 구매를 완료한다.

- 회원은 구매확정이 된 제품에 한해 리뷰를 남길 수 있다.
- 회원은 리뷰 등록시, 별점(1~5)과 리뷰내용, 이미지(최대 하나)를 등록을 한다.
...
두 개체의 관계가 n:m인 경우 - 관계에 속성이 없는 경우
	=> 새 릴레이션을 추가 후, 대리키를 기본키로 만들고, 각 개체의 기본키들을 새 릴레이션의 외래키로 지정
    
-- 관계에 속성이 있는 경우 --
두 개체의 관계가 1:1, 1:n, n:m 상관없음
	=> 새 릴레이션을 추가 후, 관계에 있는 속성을 새 릴레이션에 추가하고,
		각 객체의 기본키들을 새 릴레이션의 외래키로 지정
*/
/*
개인이 운영하는 쇼핑몰 사이트 구축을 위한 요구사항 정리
...
리뷰(리뷰번호(PK), 내용, 이미지, 아이다(FK), 옵션번호(FK))
