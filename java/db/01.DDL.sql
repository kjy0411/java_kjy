...
*/
/*
DB 생성	: MYSQL에서는 DB와 스키마가 같은 것으로 인식
create schema DB명;
create database DB명;
...
테이블 생성
 ...
 - 테이블당 최대 1개만 설정
 - dafault는 not null인 경우 기본값을 설정할 때 사용
...
create table 테이블명(
    ...
    foreign key(속성명) references 테이블명(속성명) [on update 제약조건 옵션 on delete 제약조건옵션],
    check(조건)
);
제약조건 옵션
...
SET NULL 	: 참조되고 있으면 기본키의 값이 변하면 참조하는 테이블의 값이 NULL로 바뀜
 - 단, 이 경우는 외래키가 NULL 허용인 경우에만 가능
NO ACTION	: 제약조건 옵션을 선택하지 않은 경우 자동으로 선택. RESTRICED와 동일
...
*/
-- 학생 테이블에 주소 속성을 추가
alter table student add adress varchar(50) not null default '';
-- 학생 테이블에 주소를 최대 40자로 수정
alter table student modify adress varchar(40) not null default '';
-- 학생 테이블 주소를 adress_detail로 수정
alter table student change adress adress_detail varchar(40) not null default '';
-- 학생 테이블 주소를 삭제
alter table student drop adress_detail;
