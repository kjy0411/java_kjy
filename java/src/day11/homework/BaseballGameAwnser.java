...
public class BaseballGameAwnser {
	...
	/**스트라이크 개수를 판별하는 메서드 => 두 배열에서 같은 번지에 있는 값들이 몇개 같은지 알려주는 메서드
	 * 매개변수 : 두 배열 => int arr1[], int arr2[]
	 * 리턴타입 : 같은 번지에 있는 값들이 몇개 같은지 => 몇개 => 정수 => int count
	 * 메서드명 : strike
	 */
	public static int strike(int arr1[], int arr2[]) {
		//배열 생성이 안된 배열이 있으면 비교할수 없어서 0을 반환
		if(arr1 == null || arr2 == null) {
			return 0;
		}
		//배열의 크기가 다른 경우 작은 크기의 배열을 기준으로 비교하기 위해 size 계산
	...
	/**볼 개수를 판별하는 메서드 => 두 배열에서 다른 번지에 있는 값들이 몇개 같은지 알려주는 메서드
	 * 매개변수 : 두 배열 => int arr1[], int arr2[]
	 * 리턴타입 : 다른 번지에 있는 값들이 몇개 같은지 => 몇개 => 정수 => int count
	 * 메서드명 : ball
	 */
	public static int ball(int arr1[],int arr2[]) {
		//배열 생성이 안된 배열이 1라도 있으면 비교할 수 없다.
		if(arr1 == null || arr2 == null) {
			return 0;
		}
		int count = 0;
		//다른 번지를 비교해서 찾아도 되지만 두 배열에서 같은 숫자의 개수에서 스트라이크 개수를 빼면 볼의 개수
		for(int tmp : arr1) {
