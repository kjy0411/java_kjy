...
public class ArrayRandomEx2 {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < 3;) {
			...
			if(!checkArr[random]) {
				...
				//i를 증가.
				i++;
				//checkArr배열에 random 번지에 있는 값을 true로 변경
		...
		while(count < 3){
			...
			/*위 반복문에서 break를 만나지 않으면 i는 count일 때 종료하기 때문에 반복문 종료 후 i는 count와 같다
		...
		//방법 3 : 방법2를 메서드로 변형
		System.out.println("===================");
		createRandonArray(1, 9, arr);
		printArray(arr);

		System.out.println("===================");
		System.out.println(contains(arr, 7, 2));
	}
	 ...
	 */
	public static void createRandonArray(int min, int max, int arr[]) {
