...
<body>
	...
	<!-- 댓글 기능 자바 스크립트 -->
	<script type="text/javascript">
		//로그인하지 않고 댓글 창을 활성화 했을 때 처리하기 위한 코드
		$('[name=co_contents]').focus(function() {
			if('${user.me_id}' == ''){
				if(confirm('댓글을 작성하려면 로그인해야합니다. 로그인을 하시겠습니까?')){
					location.href='<c:url value="/member/login"/>'
				}
				$(this).blur();
				return;
		...
		//게시글이 화면에 출력되고 이어서 댓글이 화면에 출력되어야 하기때문에 이벤트 등록 없이 바로 호출
		getCommentList(cri);
