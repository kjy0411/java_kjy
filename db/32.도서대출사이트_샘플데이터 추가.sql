...
-- 500.ABC123 해리포터와 마법사의 돌 1 J.K. 롤링 저자/강동혁 엮은이 문학수첩, 일반도서
...
('500.ABC123 V1 1', '외국소설', '해리포터와 마법사의 돌 1', '이지스퍼블리싱', '저자 : J.K. 롤링/엮은이 : 강동혁', 4, 1);
...
-- QWE123회원이 자바의 정석을 예약했을 때 적용해야하는 쿼리(자바의 정석 도서번호 : 101.1ABC12)
...
SET 
    BO_POSSIBLE_RESERVATION = (SELECT 
        ...
        WHERE
            BS_DESC = '예약 중')
WHERE
    BO_NUM = '101.1ABC12'; -- BOOK 테이블을 UPDATE 할 때 BOOK에서 조회한 값을 가져올 수 없다. 
...
SET
	ME_APPLY_OVERDUE = (SELECT 
			CASE
				WHEN ME_BOOK_COUNT = 0 AND ME_TOTAL_OVERDUE != 0
				THEN DATE_FORMAT(DATE_ADD(NOW(), INTERVAL ME_TOTAL_OVERDUE DAY), '%Y-%m-%d')
				ELSE NULL
			END AS T
		FROM
