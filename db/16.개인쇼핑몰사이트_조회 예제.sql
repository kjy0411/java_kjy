...
-- KH가 제품명에 포함된 제품 중에서 1페이지에 해당하는 제품들을 검색하는 쿼리(한 페이지에는 제품이 최대 5개)
SELECT
	*
FROM
	PRODUCT
...
-- KH가 제품명에 포함된 제품 중에서 2페이지에 해당하는 제품들을 검색하는 쿼리(한 페이지에는 제품이 최대 5개)
SELECT
	*
FROM
	PRODUCT
...
FROM
	...
	`OPTION` ON PR_CODE = OP_PR_CODE
		LEFT JOIN
	ORDER_LIST ON OP_NUM = OL_OP_NUM
		LEFT JOIN
