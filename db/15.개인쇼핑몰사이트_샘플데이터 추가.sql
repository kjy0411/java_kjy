...
-- 제품코드가 ABC001인 제품 중에서 유선제품이 10개 들어오고, 무선제품이 5개 들어왔을 때 사용해야하는 쿼리
UPDATE `OPTION`
	SET
		OP_AMOUNT = 10
	WHERE
...
WHERE
	OP_PR_CODE = 'ABC001' AND OP_NAME = '유선';
-- abc123회원이 ABC001 제품중 유선을 1개 장바구니에 담았을 때 쿼리
UPDATE BASKET
	SET
		BA_AMOUNT = 1
-- 	WHERE
...
-- 제품 코드가 ABC002, 제품명 KH키보드, 제품상세가 KH키보드, 제품 가격이 30000원
-- 제품 종류가 '기계식', '일반'인 제품을 등록하는 쿼리
-- 1. 제품추가
INSERT INTO PRODUCT VALUES ('ABC002', 'KH키보드', 'KH키보드', 30000);
-- 2. 옵션 추가
INSERT INTO `OPTION`(OP_NAME, OP_PR_CODE) VALUES('기계식', 'ABC002'),('일반', 'ABC002');

-- 제품 코드가 ABC002, 종류가 기계식인 제품이 100개,
-- 제품 코드가 ABC002, 종류가 일반인 제품이 50개 입고 됐을 때의 쿼리
UPDATE `OPTION`
	SET
...
-- ABC002제품 중 기계식을 3개 장바구니에 담았을 때 쿼리
INSERT INTO BASKET(BA_AMOUNT, BA_ME_ID, BA_OP_NUM)
	SELECT
		2, 'qwe123', OP_NUM
	FROM `OPTION`
...
-- 배송지를 집으로, 경기도 안산시 안산구 123, 상세는 없음, 우편번호는 12312를 등록 후 구매
-- 0. 배송지 테이블에 추가
INSERT INTO ADDRESS(AD_NAME, AD_ADDR, AD_ADDR_DETAIL, AD_POST, AD_ME_ID)
