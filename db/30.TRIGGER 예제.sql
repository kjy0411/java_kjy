/*
트리거 : 이벤트에 반응에 자동으로 실행되는 작업
=> 데이터 일관성을 유지하기 위해
트리거 예시
- 쇼핑몰에서 회원이 주문을 완료하면 주문테이블에 데이터가 추가 => 옵션 수량이 변경
트리거 특징
- OLD와 NEW를 통해 이벤트가 발생된 데이터를 접근할 수 있다.
	- INSERT 이벤트에는 이전 에디터가 없기 때문에 NEW를 통해서만 추가된 데이터에 접근할 수 있다.
...
DELETE FROM BOARD WHERE BO_NUM = 2;

-- 회원이 제품을 주문했을 때 필요한 트리거
-- 주문 테이블에 데이터 추가 => 주문 리스트에 데이터 추가 => 각 주문 리스트에 맞는 옵션에 수량이 변경
...
BEGIN
    ...
    IF NEW.OR_STATE = '구매 확정' THEN
		UPDATE MEMBER
		SET
			ME_POINT = ME_POINT + NEW.OR_SAVE_POINT
		WHERE
			ME_ID = NEW.OR_ME_ID;
		INSERT POINT(PO_CONTENT, PO_AMOUNT, PO_ME_ID)
		VALUES('제품 구매에 의한 적립', NEW.OR_SAVE_POINT, NEW.OR_ME_ID);
    END IF;
END //
DELIMITER ;

UPDATE `ORDER` SET OR_STATE = '구매 확정' WHERE OR_NUM = 3;